...
export default defineUserConfig({
  ...
  theme: FileList([
    ...
    {
      mountPath: "/Dev-C++/Embarcadero Dev-C++",
      analysis: githubReleasesFilesAnalysis({ user: "Embarcadero", repository: "Dev-Cpp" , authorizationToken: process.env.githubToken,}),
      // 下载代理配置,支持多个平台，参考:https://jjaw.cn/2024/8/3/flist-config-porxy/
      // 这个是为了解决github的国内下载慢的问题，和跨域问题，建议配置，不然pdf，txt，md等文件因为跨域无法预览
      // 如果你使用的不是 cloudflare Pages 部署需要删掉这一行，因为如果不是cloudflare Pages部署，这个代理是无法正常工作的
      downProxy: netlifyDownProxy(),
    ...
    {
      mountPath: "/Dev-C++/XiaoLoong Dev-C++",
      analysis: giteeReleasesFilesAnalysis({
        user: "devcpp",
        repository: "devcpp",
        direction: "desc"
    ...
    {
      mountPath: "/TDM-GCC (MinGW)",
      analysis: githubReleasesFilesAnalysis({ user: "jmeubank", repository: "tdm-gcc" , authorizationToken: process.env.githubToken,}),
      // 下载代理配置,支持多个平台，参考:https://jjaw.cn/2024/8/3/flist-config-porxy/
      // 这个是为了解决github的国内下载慢的问题，和跨域问题，建议配置，不然pdf，txt，md等文件因为跨域无法预览
      // 如果你使用的不是 cloudflare Pages 部署需要删掉这一行，因为如果不是cloudflare Pages部署，这个代理是无法正常工作的
      downProxy: netlifyDownProxy(),
    ...
    {
      mountPath: "/tuack-ng",
      analysis: githubReleasesFilesAnalysis({ user: "tuack-ng", repository: "tuack-ng" , authorizationToken: process.env.githubToken,}),
      // 下载代理配置,支持多个平台，参考:https://jjaw.cn/2024/8/3/flist-config-porxy/
      // 这个是为了解决github的国内下载慢的问题，和跨域问题，建议配置，不然pdf，txt，md等文件因为跨域无法预览
      // 如果你使用的不是 cloudflare Pages 部署需要删掉这一行，因为如果不是cloudflare Pages部署，这个代理是无法正常工作的
      downProxy: netlifyDownProxy(),
    ...
    {
      mountPath: "/八中重开模拟器（recallbj8z）",
      analysis: githubReleasesFilesAnalysis({ user: "liuenyin", repository: "recallbj8z" , authorizationToken: process.env.githubToken,}),
      // 下载代理配置,支持多个平台，参考:https://jjaw.cn/2024/8/3/flist-config-porxy/
      // 这个是为了解决github的国内下载慢的问题，和跨域问题，建议配置，不然pdf，txt，md等文件因为跨域无法预览
      // 如果你使用的不是 cloudflare Pages 部署需要删掉这一行，因为如果不是cloudflare Pages部署，这个代理是无法正常工作的
      downProxy: netlifyDownProxy(),
    ...
    {
      mountPath: "/OICPP",
      analysis: githubReleasesFilesAnalysis({ user: "mywwzh", repository: "oicpp" , authorizationToken: process.env.githubToken,}),
      // 下载代理配置,支持多个平台，参考:https://jjaw.cn/2024/8/3/flist-config-porxy/
      // 这个是为了解决github的国内下载慢的问题，和跨域问题，建议配置，不然pdf，txt，md等文件因为跨域无法预览
      // 如果你使用的不是 cloudflare Pages 部署需要删掉这一行，因为如果不是cloudflare Pages部署，这个代理是无法正常工作的
      downProxy: netlifyDownProxy(),
    ...
    {
      mountPath: "/TermEdit",
      analysis: githubReleasesFilesAnalysis({ user: "ILoveScratch2", repository: "TermEdit" , authorizationToken: process.env.githubToken,}),
      // 下载代理配置,支持多个平台，参考:https://jjaw.cn/2024/8/3/flist-config-porxy/
      // 这个是为了解决github的国内下载慢的问题，和跨域问题，建议配置，不然pdf，txt，md等文件因为跨域无法预览
      // 如果你使用的不是 cloudflare Pages 部署需要删掉这一行，因为如果不是cloudflare Pages部署，这个代理是无法正常工作的
      downProxy: netlifyDownProxy(),
